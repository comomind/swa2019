...
public class TestActivity2 extends AppCompatActivity {
    public static final String TAG = TestActivity2.class.getName().toString();

    private final int JITTER_BUFFER_JITTER = 30;
    private final int JITTER_BUFFER_PERIOD = 200;

    private final int RECEIVE_PORT1 = 5123;
    private final int RECEIVE_PORT2 = 5124;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        JitterBuffer jitterBuffer1 = new JitterBuffer(JITTER_BUFFER_JITTER, JITTER_BUFFER_PERIOD, audioCodec.getSampleRate());
        JitterBuffer jitterBuffer2 = new JitterBuffer(JITTER_BUFFER_JITTER, JITTER_BUFFER_PERIOD, audioCodec.getSampleRate());

        audioRecorder = new AudioRecorder(audioCodec);
        audioRecorder.addRecorderQueue("test1", recorderQueue);

        audioPlayer = new AudioPlayer(getApplicationContext(), audioRecorder.getAudioSessionId(), audioCodec);
        audioPlayer.addJitterBuffer("test1", jitterBuffer1);
        audioPlayer.addJitterBuffer("test2", jitterBuffer2);

        audioIo1 = new AudioIo(getApplicationContext(), audioCodec, jitterBuffer1, recorderQueue);
        audioIo2 = new AudioIo(getApplicationContext(), audioCodec, jitterBuffer2, recorderQueue);
